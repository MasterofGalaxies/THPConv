...
#include <revolution/iop_ip.h>
#include <revolution/iop_ip6.h>
#include <revolution/iop_ipdns.h>
#include <revolution/iop_netif.h>
#include <revolution/iop_icmp.h>
#include <revolution/IPStat.h>
...
#define SO_SOCKET_MAX       24          // maximum number of open sockets
...
#define SO_ERR_PPPoE_TIMEOUT        -103    // Could not find any access concentrator.
#define SO_ERR_PPPoE_SERVICE_NAME   -104    // The requested Service-Name request 
                                            // could not be honored.
#define SO_ERR_PPPoE_AC_SYSTEM      -105    // The access concentrator experienced some error.
#define SO_ERR_PPPoE_GENERIC        -106    // An unrecoverable error occured.
...
#define SO_ERR_PPP_TERMINATED       -110    // The access concentrator terminated 
                                            // the PPPoE session.
#define SO_ERR_ADDR_COLLISION       -111    // Duplicate IP address
#define SO_ERR_LINK_DOWN            -112    // Link down

// network resource manager not available at IOP
#define SO_ERR_NETRM_NOTAVAIL       0x80000000
...
{
    int         fd;
    int         events;         // input event flags
    int         revents;        // output event flags
} SOPollFD;
...
typedef struct SOConfig
{
    ...
    OSTime      r2 __attribute__ ((aligned (8)));
                                    // default TCP total retransmit timeout value (default 100 sec)
...
void  SOInitInterface( void );
int   SOiStartup     ( const SOConfig* config );
int   SOiCleanup     ( void );

int   SOInit         ( const SOLibraryConfig *config );
int   SOFinish       ( void );

int   SOSetInterface ( int interfaceId );
int   SOGetInterfaceOpt( int level, int optname, void* optval, int* optlen );
int   SOSetInterfaceOpt( int level, int optname, const void* optval, int optlen );
...
#define SO_CONFIG_IP_ADDR                           0x400a
#define SO_CONFIG_IP_GATEWAY                        0x400b
#define SO_CONFIG_IP_NETMASK                        0x400c
#define SO_CONFIG_IP_REFRESH_ROUTE                  0x400d
